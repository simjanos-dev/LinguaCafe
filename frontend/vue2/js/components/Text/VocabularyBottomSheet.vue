<template>
    <v-card id="vocab-bottom-sheet" color="foreground" @mouseup.stop="">
        ...
        <v-card-text
            class="searchBoxBorder mx-auto pa-3 rounded-lg border"
            :class="[tab === 2 ? 'mt-2' : '']"
        >
            <v-tabs-items v-model="tab">
                <!-- Main tab -->
        ...
        <v-card-actions class="d-flex flex-column">
            ...
            <div class="w-100 d-flex justify-space-around mb-2">
                ...
                <template v-if="tab === 0">
                    ...
                    <v-btn icon :disabled="!$props.textToSpeechAvailable" @click="textToSpeech"
                        ><v-icon>mdi-bullhorn</v-icon></v-btn
                    >
                    <v-btn icon :disabled="!inflections.length" @click="tab = 1"
                        ><v-icon>mdi-list-box</v-icon></v-btn
                    >
                    <v-btn icon title="Add or edit image" v-bind="attrs" v-on="on" @click="tab = 2">
    ...
    methods: {
        ...
        addDefinitionToInput(definition) {
            if (
                ...
                this.translationText[this.translationText.length - 1] !== ';'
            ) {
